...
export const DeviceSelector: React.FC<DeviceSelectorProps> = ({
          ...
          try {
            ...
            const filteredDevices = newDeviceInfos
              ...
              .map((device, index) => ({
                ...
                label:
                  device.label || `${type === 'camera' ? 'Camera' : 'Microphone'} ${index + 1}`,
  ...
  return (
    <div className="device-selector-menu absolute z-50 bg-white rounded-lg shadow-xl border border-gray-200 py-2 min-w-[280px] bottom-full mb-2 left-1/2 -translate-x-1/2">
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {devices.length === 0 ? (
          <div className="px-4 py-3 text-sm text-gray-500 text-center">No {type}s found</div>
        ) : (
          devices.map((device) => {
            ...
            return (
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span
                    className={`truncate max-w-[200px] ${isSelected ? 'font-semibold text-blue-700' : ''}`}
                  >
                    {device.label}
                  </span>
                </span>
