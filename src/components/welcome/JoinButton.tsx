...
export const JoinButton: React.FC<JoinButtonProps> = ({ isOnline, onClick, disabled, isChecking, isCheckingOnlineStatus }) => {
  ...
  if (isCheckingOnlineStatus) {
    return (
      <button
        disabled
        className="w-full py-4 sm:py-5 rounded-2xl bg-gradient-to-r from-gray-400 to-gray-500 text-white font-bold cursor-not-allowed text-sm sm:text-base shadow-xl border-2 border-gray-300/50 relative overflow-hidden"
      >
  ...
  if (isChecking) {
    return (
      <button
        disabled
        className="w-full py-4 sm:py-5 rounded-2xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-bold cursor-not-allowed text-sm sm:text-base shadow-xl border-2 border-blue-400/50 relative overflow-hidden"
      >
        <div className="absolute inset-0 bg-black/10"></div>
        <div className="relative flex flex-col items-center justify-center gap-2">
          <span className="flex items-center gap-2">
